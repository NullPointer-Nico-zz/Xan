...
class MuteCommand(commands.Cog):
    ...
    @commands.command()
    async def mute(self, ctx, args=None, *, member: discord.Member=None):
        ...
        if ctx.message.author.guild_permissions.manage_messages:
            ...
            if member == ctx.message.guild.owner:
                await ctx.send(f'**{member.name} ist der Server Owner! Den kann ich nicht Muten!**')
                return

            if member == ctx.message.author:
                await ctx.send(f'**{ctx.message.author.name}Hast du greade versucht dich selber zu muten? War woll nix!**')
                return
